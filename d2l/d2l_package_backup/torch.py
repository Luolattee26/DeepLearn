...
#@save
# 上面虽说torch提供了一个RNN，但是那只是一个RNN的单个循环层
# 所以其他的我们还是需要自己来定义的
# 其实torch给的那个RNN，就是我们上面手动实现的那个rnn函数一样的东西
# 但是要注意有个差别，我们手动实现的rnn函数带有输出层，也就是h到y这一层
# 但是torch的RNN是没有这一层的，torch的RNN只有x到h这一层，后面的h到y需要我们自己去定义的
...
class RNNModelScratch: #@save
    ...
    def __init__(self, vocab_size, num_hiddens, device,
        ...
        # 这里可以发现，我们定义了一个self.forward_fn属性
        # 这里面要传入的其实就是我们上面定义的rnn函数
        # 但是我们上面定义的那种是最简单的RNN，后面LSTM啥的不一样（在参数更新上），所以这里写的比较通用一点
        self.init_state, self.forward_fn = init_state, forward_fn

    def __call__(self, X, state):
        # 这里要注意，onehot出来的其实是一个int，我们要手动转化一下
...
# 接下来，我们定义一个预测函数
# prefix指的是，我给你一个句子的开头，然后让你预测下面的，其实就是续写
# num_preds是需要预测多长的东西
# net训练好的网络
def predict_ch8(prefix, num_preds, net, vocab, device):  #@save
    ...
    # 这里也就是，我们每次新预测出来的词，然后再把它当做输入去预测下一个词
    get_input = lambda: torch.tensor([outputs[-1]], device=device).reshape((1, 1))
    # 这里的意思是，我们做NLP模型的续写，我们一开始给你的prefix肯定是标准答案，我们不可能去更改的
    # 但是对于这些标准答案，我们可以用来生成隐状态
    # 从prefix的第1个token开始，就能逐渐生成前面的隐状态了
    # 第0个隐状态肯定是我们初始化的
    # 然后再把token放进输出里面，这些都是直接有，不需要预测的东西
    # 这被叫做预热期
    for y in prefix[1:]:
...
def train_epoch_ch8(net, train_iter, loss, updater, device, use_random_iter):
    ...
    for X, Y in train_iter:
        if state is None or use_random_iter:
            ...
            # 还记得我们前面说过两种抽样方法的差别
            # 如果我们这里用use_random_iter，意味着我不同batch之间的sequence是不连续的，那我的隐状态h肯定不存在这之间的时序信息
            # 所以用随机采样，我们在每个batch之间也会对隐状态h进行初始化
            state = net.begin_state(batch_size=X.shape[0], device=device)
        else:
            if isinstance(net, nn.Module) and not isinstance(state, tuple):
                # state对于nn.GRU是个张量
                # 我们前面的隐状态反正不用更新了，我们detach调就好
                state.detach_()
            else:
        ...
        # 我们前面对输出做了一个cat操作，其实就是为了这里做loss的时候，转化为一个标准的多分类问题
...
# 大的train函数，和前面套路都是一样的
# 没啥特殊的，只是我们又弄了个predict函数的封装
# 就是训练一段时间我们调用一下，看一下time traveller续写是啥效果
#@save
...
# 这里就是我们要如何针对文本数据，生成可供训练的序列数据
# 这里就是针对整个序列，每次去随机采样一个一定长度的序列，作为x，后一个token作为y
# 实际上这里是有一些实现的技术细节的，详见李沐老师的视频
...
def predict_ch3(net, test_iter, n=6):  #@save
    ...
    for X, y in test_iter:
        break
    trues = d2l.get_fashion_mnist_labels(y) # 之前实现的函数在这用上了啊......
...
class __Vocab:
    ...
    def __init__(self, tokens=[], min_freq=0, reserved_tokens=[]):
        ...
        self.idx_to_token = list(sorted(set(['<unk>'] + reserved_tokens + [
            token for token, freq in self.token_freqs if int(freq) >= int(min_freq)])))
        self.token_to_idx = {token: idx
                             for idx, token in enumerate(self.idx_to_token)}
...
#@save
class EncoderDecoder_simple(nn.Module):
    """编码器-解码器架构的基类"""
    def __init__(self, encoder, decoder, **kwargs):
        super(EncoderDecoder_simple, self).__init__(**kwargs)
        self.encoder = encoder
        self.decoder = decoder
...
def train_seq2seq(net, data_iter, lr, num_epochs, tgt_vocab, device):
    ...
    for epoch in range(num_epochs):
        ...
        for batch in data_iter:
            ...
            # 这里我们用了一个填充bos，代表句子的开头
            # 其实就是为了decoder的输出
            # 因为序列模型都是给一个词预测下一个词
            # 所以我们手动在最前面给一个bos，最后一个词删掉，这样训练的时候decoder给出的结果就刚好是fix整个真实的y了
            # 训练算法的核心就是，怎么构造decoder的输入和输出
